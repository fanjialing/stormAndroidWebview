...
import com.exanple.exception.CustomException;
...
//	private int[] mDrawableIds = new int[] { R.drawable.tab_weixin_pressed,
...
//	R.drawable.tab_settings_normal };
	...
	public MainTabWidget(Context context, AttributeSet attrs, int defStyle) {
		...
		if (null == mLabels || mLabels.length <= 0) {
			try {
				...
				LogUtils.i(TAG, MainTabWidget.class.getSimpleName() + " 出错");
			}
			a.recycle();
			return;
		}
	...
	private void init(final Context context) {
		...
		for (int i = 0; i < size; i++) {
			...
			if (i == 0) {
				...
				itemName.setTextColor(Color.rgb(0,124,195));
	...
	public void setTabsDisplay(Context context, int index) {
		...
		for (int i = 0; i < size; i++) {
			...
			if ((Integer) (checkedTextView.getTag()) == index) {
				...
				checkedTextView.setTextColor(Color.rgb(0,124,195));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		height = Math.max(this.getBackground().getIntrinsicHeight(),
				heightSpecSize);
		setMeasuredDimension(width, 135);
	}
